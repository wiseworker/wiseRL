...
from wiserl.utils.replay_buffer import SharedReplayBuffer
from wiserl.agent.mat_agent.mat_agent import TransformerPolicy as Policy
...
class Runner(Actor):
    ...
    def compute(self):
        ...
        if self.buffer.available_actions is None:
            ...
            next_values = self.trainer.get_values(self.policy, self.buffer)

        next_values = np.array(np.split(_t2n(next_values), self.n_rollout_threads))
        self.buffer.compute_returns(next_values, self.trainer.normalizer())
    ...
    def load(self, episode):
        """Load policy's actor and critic networks."""
        self.policy.load(self.save_dir, episode)
