...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        # defined the high level state for the RIME Algorithm
        high_state = np.zeros(6)
        high_state[0] = agent.size
        high_state[1] = agent.accel
        high_state[2] = agent.initial_mass
        high_state[3] = agent.density
        high_state[4] = world.damping
        high_state[5] = entity_size
        ...
        other_pos = []
        high_level_state = []
        high_level_state.append(high_state)
        for other in world.agents:
            if other is agent: continue
            comm.append(other.state.c)
            other_pos.append(other.state.p_pos - agent.state.p_pos)
        if not agent.adversary:
            return np.concatenate([agent.state.p_vel] + [agent.goal_a.state.p_pos - agent.state.p_pos] + [agent.color] + entity_pos + entity_color + other_pos + high_level_state)
        else:
            ...
            return np.concatenate([agent.state.p_vel] + entity_pos + other_pos + high_level_state)
