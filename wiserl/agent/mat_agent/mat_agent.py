...
from wiserl.utils.utils import update_linear_schedule, get_gard_norm, huber_loss, mse_loss, check, get_shape_from_obs_space
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if act_space.__class__.__name__ == 'Box':
            ...
            print('Action_space: Continuous')
        else:
            self.action_type = 'Discrete'
            print('Action_space: Discrete')
        ...
        from wiserl.net.transformer_net import MultiAgentTransformer as MAT
    ...
    def load(self, load_dir, episode):
        self.transformer.load_state_dict(torch.load(str(load_dir) + "/transformer_" + str(episode) + ".pt"))
...
class MATTrainer(Agent):
    ...
    def __init__(self,
        ...
        if self._use_valuenorm:
            self.value_normalizer = ValueNorm(1)
        else:
    ...
    def get_actions(self, policy, buffer, step):
        value, action, action_log_prob, rnn_state, rnn_state_critic = policy.get_actions(np.concatenate(buffer.share_obs[step]),\
    ...
    def get_values(self, policy, buffer):
        next_value = policy.get_values(np.concatenate(buffer.share_obs[-1]),
