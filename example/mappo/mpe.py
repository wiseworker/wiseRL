...
class GymRunner(Runner):
        ...
        while total_steps < self.cfg.max_train_steps:
            ...
            while not done:
                ...
                if total_steps % 2000 ==0:
                    evaluate_num += 1
                    evaluate_reward = self.evaluate_policy(self.cfg, self.env_evaluate, self.agents, self.state_norm)
    ...
    def get_actions(self,observations):
        ...
        for index, agent in enumerate(observations): 
            a, a_logprob = self.agents[index].choose_action(observations[agent])  # Action and the corresponding log probability
            actions[agent]=a
    ...
    def store(self,observations, actions, logprobs, rewards, observations_, dw, done ):
        for index, agent in enumerate(observations):
            ...
            self.replay_buffer[index].store(observations[agent], actions[agent], logprobs[agent], rewards[agent], s_, dw, done)

    def agent_update(self,total_steps):
        for i in range(self.cfg.num_agent): 
            print("update")
            self.agents[i].update(self.replay_buffer[i], total_steps)
            self.replay_buffer[i].count = 0
