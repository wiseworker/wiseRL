...
class GymRunner(Runner):
    def __init__(self, args, local_rank=0):
        ...
        if self.local_rank == 0:
            wise_rl.make2_agent(name=self.agent_name, agent_class=PPOAgent, sync=True, **vars(self.config))    
    ...
    def run(self):
        ...
        while total_steps < self.config.max_train_steps:
            ...
            train_info.append(episode_reward)
            if episode_num % 2 == 0: 
                ...
                plt.ylabel('Rewards')
                plt.grid(True)
                plt.savefig('PPOdiscrete-Rewards')
