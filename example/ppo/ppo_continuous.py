...
class GymRunner(Runner):
    def __init__(self, args=None, local_rank=0):
        ...
        self.cfg.state_dim = self.env.observation_space.shape[0]
        self.cfg.action_dim = self.env.action_space.n
        self.cfg.max_episode_steps =self.env._max_episode_steps  # Maximum number of steps per episode
        if local_rank == 0:
            wise_rl.make_agent(name=self.agent_name, agent_class=PPO2Agent, net_cfg=self.net_cfg,cfg=args,sync=False)
    ...
    def run(self):
        ...
        replay_buffer = ReplayBuffer(self.cfg)
...
if __name__ == '__main__':
    parser = ("Hyperparameter Setting for PPO-discrete")
