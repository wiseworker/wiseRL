...
from wiserl.core.wise_rl import  makeRunner,makeAgent,getRunner,getAgent
import config
import gym
import time
import ray
...
class GymRunner(Runner):
    def __init__(self,local_rank=0):
        ...
        if local_rank ==0:
            ...
            makeAgent(DQNAgent,DQNNet,N_STATES,N_ACTIONS,config)
            self.agent = getAgent(DQNAgent)
        else:
            self.agent = getAgent(DQNAgent)
        ...
        for i_episode in range(config.MAX_EPOCH):
            ...
            while True:
                ...
                if done:
                    ...
                    print('time', round((end-start),2),' Ep: ', i_episode, ' |', 'Ep_r: ', round(ep_r, 2))
                    break
                s = s_  
